...
class Grid(nx.DiGraph):
    ...
    def find_edges_of(self, this):
        candidates = self.get_candidate_neighbors(this)
        candidates['adjacent'] = candidates['adjacent']
        edges = []
        edges += [ (this, x) for x in candidates['adjacent'] if self.node[x]['value'] % 2 == 0 ]
